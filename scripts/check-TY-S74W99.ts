...
async function checkOrder() {
  console.log("=== Checking Order TY-S74W99 ===\n");

  const order = await prisma.order.findUnique({
    where: { orderNumber: "TY-S74W99" },
  });
  ...
  try {
    ...
    if (data.data && data.data.acknowledgedDateMs) {
      ...
      console.log("⏳ Payment NOT in Bakong yet");
      console.log("From:", data.data.fromAccountId);
      console.log("To:", data.data.toAccountId);
      console.log("Created:", new Date(data.data.createdDateMs).toISOString());
      console.log("Amount:", data.data.amount, data.data.currency);
    } else {
      console.log("❌ No data from Bakong");
    }
