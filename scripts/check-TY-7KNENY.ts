...
async function checkOrder() {
  console.log("=== Checking Order TY-7KNENY ===\n");

  const order = await prisma.order.findUnique({
    where: { orderNumber: "TY-7KNENY" },
  });
  ...
  try {
    ...
    if (data.data && data.data.acknowledgedDateMs) {
      ...
      if (order.status === "PENDING") {
        console.log("\n⚠️  Auto-detection FAILED again!");
        console.log("Updating to PAID manually...");
      ...
      console.log("⏳ Payment not yet in Bakong");
    }
